...
- **Path-focused hop cards & diagnostics:** Hop cards show traced path ports by default (downlink to previous hop / endpoint and uplink to next hop) with full per-port diagnostic sweeps available behind an expander (PVID, tagged and untagged VLAN semantics, allowed VLANs, STP state, link speed, and neighbor identity).
...
- **Prerequisites & Fallback:** Requires SNMP v2c read access on the managed switches. When a switch or upstream neighbor does not advertise a management IP via LLDP/CDP (common on UniFi and some other vendors), LinkSight offers it as a path candidate and auto-resolves the management IP from its chassis MAC via an ARP sweep of the local subnet; if that also fails, the user can enter the switch IP manually. If an intermediate switch does not have SNMP enabled or does not respond, discovery stops gracefully at that hop and presents the last reachable segment.
